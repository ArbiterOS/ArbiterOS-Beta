...
[^1]: This can be related to agent's paradiam. For ReAct agents, only `REASON` is present. For Plan-and-execute agents, `PLAN` is added. For Reflexion agents, `CRITIQUE` is added...
[^2]: Atomic actions marked with an asterisk means they are more likely to be implemented in code instead of as a tool available to the agent (but still possible). Thus may be hard to be captured by our endpoint gateway design...
